...
if (isset($_POST["btnNuevoRegistro"])) {
    ...
    if ($error_form) {
        ...
        if ($_FILES["archivo"]["name"] != "") {
            ...
            $nombre_nuevo = $_POST["usuario"] . "." . $ext;
            $ruta_destino = "../images/" . $nombre_nuevo;
            @$var = move_uploaded_file($_FILES["archivo"]["tmp_name"], "../images/" . $ruta_destino);
            if ($var) {
                try {
                    $consulta = "UPDATE usuarios SET foto = ? WHERE usuario = ?";
                    $sentencia = $conexion->prepare($consulta);
                    $sentencia->execute([$nombre_nuevo, $_POST["usuario"]]);
                } catch (PDOException $e) {
                    $sentencia = null;
                    $conexion = null;
