<?php
//teoria de sesisones
session_name("teoria_pdo");// le ponemos un nombre para cada aplicación
session_start(); // la inicio
// creo los valores de la sesion
$_SESSION["usuario"];
//destruyo la sesion
session_destroy();
// destruyoo las variables 
session_unset();
...
<body>
    ...
    */
    ...
    try{
        ...
        $sentencia=$conexion->prepare($consulta); // prepare() es un metodo que recibe la consulta
        $sentencia->execute([$usuario,$clave]);// execute solo admite arrays, $sentencia es un objeto
    ...
    if($sentencia->rowCount()<=0) // si no hay tuplas
    {
        echo "<p>No hay usuario con esas credenciales en la base de datos</p>";
    }
    else
    {
        $tupla=$sentencia->fetch(PDO::FETCH_ASSOC);//otras constantes PDO::FETCH_NUM, PDO::FETCH_OBJECT  función fetch() recojo los datos en este caso nos traemos un array asociativo
 ...
 */
    ...
    if ($sentencia->rowCount() <= 0) {
        ...
        $respuesta = $sentencia->fetchall(PDO::FETCH_ASSOC); //otras constantes PDO::FETCH_NUM, PDO::FETCH_OBJECT  la función fectall() para obtener un array de usuarios
 ...
 */
    ...
    echo "<p>El Usuario  insertado con exito con la id_usuario: <strong>" . $conexion->lastInsertId() . "</strong></p>"; //lastInsertId() me da la id de la ulitma insercción
