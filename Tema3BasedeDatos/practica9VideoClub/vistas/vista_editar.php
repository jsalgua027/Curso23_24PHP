...
if (mysqli_num_rows($resultado) > 0) {
    ...
    echo "<label for='archivo'>Cambiar de carátula de la película:</label>
    <input type='file' name='archivo' id='archivo' accept='Img/*'><br>";
    echo '<button type="submit" name="btnConEditar" >Editar película</button>';
    echo '<button name="btnAtras" >Atras</button>';
    echo "</p>";
    echo "<p id='foto'>";
    echo "<strong>Carátura Actual</strong><br>";
    echo "<img src='Img/" . $datos_pelicula["caratula"] . "' name='foto' title='caratula' id='foto-caratula'><br>";
    echo '<button type="submit" name="btnEliminarCaratula" >Eliminar caratura</button>';
    echo "</p>";
    echo "</div>";
} else {
